        ...
        <>
            ...
            Comments
              ...
              Comments will appear here once all approvers have responded.
                ...
                {/* ── Comment threads ───────────────────────────────────────────── */}
                <Box sx={{
                    flex: 1, overflowY: "auto", px: 2, py: "4px", pb: "12px",
                    display: "flex", flexDirection: "column", gap: "16px",
                    ...(awaitingApprovers && tab === "unresolved" ? { display: "none" } : {})
                }}>
                    {threads.map(thread => {
...
// ─── Scene thumbnail ──────────────────────────────────────────────────────────
function SceneThumbnail({ index, selected, headingText, subheadingText, footnoteText, onClick }: {
    index: number; selected: boolean; headingText?: string; subheadingText?: string;
    footnoteText?: string; onClick?: () => void;
}) {
    return (
...
export default function StudioPage({
    videoTitle, initialHeadingText, initialSubheadingText, approverNames,
    onNavigateToVideoPage, onNavigateToLibrary, onRequestReapproval,
    onHeadingChange, onSubheadingChange, openCommentsOnMount, triggerOpenComments,
    notifications, initialThreads, initialPermSettings, onPermChange,
    awaitingApprovers, onEditAttempt
}: Props) {
            ...
            <Box sx={{ flex: 1, display: "flex", overflow: "hidden" }}>
                ...
                <Box sx={{ flex: 1, bgcolor: s.editorBg, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                    ...
                    <Box sx={{
                        ...
                        <Box
                            onClick={() => {
                                setHeadingSelected(false); setSubheadingSelected(false); setFootnoteSelected(false); 
                            }}
                            sx={{
                                        ...
                                        <Box sx={{
                                            ...
                                            >
                                                <Typography sx={{
                                                    ...
                                                    fontSize: "4.5cqw", color: s.textPrimary,
