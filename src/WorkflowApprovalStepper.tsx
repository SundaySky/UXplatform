...
export default function WorkflowApprovalStepper({ phase, approvers }: Props) {
  ...
  function statusOf(stepIdx: number): StepStatus {
    if (phase === 0) return stepIdx === 0 ? 'active' : 'pending'
    if (phase === 1) {
  ...
  if (approverNames && phase >= 1 && phase <= 3) subtitleParts.push(approverNames)
  if (respondedInfo)                              subtitleParts.push(respondedInfo)
